...
- **Namespace-qualified module APIs are also valid.** Unqualified exports must carry their domain
  identity. Generic exports such as `make` and `layer` are acceptable in a domain-named module when
  consumers use a stable, explicit namespace import and the filename identifies the concept:
  `CloudflareWorkspaceEnvironmentManager.layer(env)`, not `layer(env)`.
