...
const reviewCommand = Command.make(
  ...
  ({ base, coordinatorModel, json, repo, reviewerModel }) =>
    Effect.gen(function* () {
      const review = yield* Review
      const onProgress = makeCompactProgressReporter((line) => process.stderr.write(`${line}\n`))
      ...
      Effect.catchAll((error) =>
        (json
          ...
          : Console.error(`hanif-agent: ${reviewErrorMessage(error)}`)
        ).pipe(
          Effect.tap(() =>
            Effect.sync(() => {
              process.exitCode = 1
...
const root = Command.make("hanif-agent", {}, () =>
  Console.log("Use 'hanif-agent review --help' to run a workflow."),
).pipe(Command.withDescription("Personal agent workflows backed by OpenCode"), Command.withSubcommands([reviewCommand]))
