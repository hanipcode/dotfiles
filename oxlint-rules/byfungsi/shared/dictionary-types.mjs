...
import {
  ...
  visibleTypeAlias
} from "./type-alias-resolution.mjs";
...
/** Collect lexical aliases and interface evidence for dictionary classification. */
...
/** Classify dictionary value contracts without rejecting meaningful nested unknown fields. */
export function classifyUnsafeDictionaryValue(valueType, environment) {
  const unsafeValue = unsafeDirectValue(valueType, environment, new Map, new Set);
  return unsafeValue === null ? null : { kind: "unsafe-dictionary", unsafeValue };
}
/** Resolve a dictionary's value contract through visible aliases and transparent wrappers. */
...
/** Classify explicit targets that erase known evidence, including Fungsi's any/empty-object ban. */
export function classifyWideningTarget(type, environment) {
  ...
  if (unwrapped.type === "TSObjectKeyword")
    return { kind: "object" };
  if (unwrapped.type === "TSTypeLiteral") {
    return unwrapped.members.some((member) => member.type === "TSIndexSignature") ? { kind: "open dictionary" } : unwrapped.members.length > 0 ? { kind: "anonymous object" } : { kind: "empty object" };
  }
  if (unwrapped.type === "TSMappedType")
...
function classifyAliasBroadTarget(type, environment, substitutions, resolvingAliases) {
  ...
  if (unwrapped.type === "TSObjectKeyword")
    return { kind: "object" };
  if (unwrapped.type === "TSTypeLiteral") {
    return unwrapped.members.some((member) => member.type === "TSIndexSignature") ? { kind: "open dictionary" } : unwrapped.members.length === 0 ? { kind: "empty object" } : null;
  }
  if (unwrapped.type === "TSMappedType") {
...
/** Recognize populated literal objects through assertion and parenthesis wrappers. */
...
/** Recognize locally established value evidence without inferring imported call signatures. */
